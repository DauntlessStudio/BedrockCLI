...
void entity::player_entity(int argc, char* argv[])
{
	...
	if (result.count("rp"))
	{
		...
		if (result.count("geo"))
		{
			...
			rp_entity["minecraft:client_entity"]["description"]["scripts"]["animate"].push_back("ctrl.custom_item.select");
			rp_entity["minecraft:client_entity"]["description"]["scripts"]["animate"].push_back({ {"custom_item.first_person.base", "v.is_first_person && v.has_custom_item"} });
			rp_entity["minecraft:client_entity"]["description"]["scripts"]["animate"].push_back({ {"custom_item.third_person.right", "!v.is_first_person && v.has_custom_item && v.aim_left_arm"} });
			rp_entity["minecraft:client_entity"]["description"]["scripts"]["animate"].push_back({ {"custom_item.third_person.left", "!v.is_first_person && v.has_custom_item && v.aim_right_arm"} });
			rp_entity["minecraft:client_entity"]["description"]["animations"]["ctrl.custom_item.select"] = "controller.animation.player.custom_item.select";
			rp_entity["minecraft:client_entity"]["description"]["animations"]["custom_item.first_person.base"] = "animation.player.custom_item.base_first_person_pose";
			rp_entity["minecraft:client_entity"]["description"]["animations"]["custom_item.third_person.right"] = "animation.player.custom_item.third_person_aim_arm.right";
			rp_entity["minecraft:client_entity"]["description"]["animations"]["custom_item.third_person.left"] = "animation.player.custom_item.third_person_aim_arm.left";
			...
			rp_anim_controller["animation_controllers"]["controller.animation.player.custom_item.select"]["initial_state"] = "no_item";
			rp_anim_controller["animation_controllers"]["controller.animation.player.root"] = nlohmann::ordered_json::parse(R"({ "initial_state" : "first_person", "states" : { "first_person" : { "animations" : [ "first_person_swap_item", { "first_person_attack_controller" : "v.attack_time > 0.0f && q.get_equipped_item_name != 'map' && !v.has_custom_item" }, { "first_person_base_pose": "!v.has_custom_item" }, { "first_person_empty_hand" : "q.get_equipped_item_name(0, 1) != 'map' && !v.has_custom_item" }, { "first_person_walk" : "v.bob_animation" }, { "first_person_map_controller" : "(q.get_equipped_item_name(0, 1) == 'map' || q.get_equipped_item_name('off_hand') == 'map') " }, { "first_person_crossbow_equipped": "q.get_equipped_item_name == 'crossbow' && (v.item_use_normalized > 0 && v.item_use_normalized < 1.0) " }, { "first_person_breathing_bob": "v.attack_time <= 0.0 || v.has_custom_item" } ], "transitions" : [ { "paperdoll" : "v.is_paperdoll" }, { "map_player" : "v.map_face_icon" }, { "third_person" : "!v.is_first_person && !q.is_saddled" } ] }, "map_player" : { "transitions" : [ { "paperdoll" : "v.is_paperdoll" }, { "first_person" : "v.is_first_person" }, { "third_person" : "!v.map_face_icon && !v.is_first_person" } ] }, "paperdoll" : { "animations" : [ "humanoid_base_pose", "look_at_target_ui", "move.arms", "move.legs", "cape" ], "transitions" : [ { "first_person" : "!v.is_paperdoll && v.is_first_person" }, { "map_player" : "v.map_face_icon" }, { "third_person" : "!v.is_paperdoll && !v.is_first_person" } ] }, "third_person" : { "animations" : [ "humanoid_base_pose", { "look_at_target" : "!q.is_sleeping && !q.is_emoting" }, { "move.arms": "!v.disable_arm_swing" }, "move.legs", "cape", { "riding.arms" : "q.is_riding && !v.disable_leg_swing" }, { "riding.legs" : "q.is_riding && !v.disable_leg_swing" }, { "holding": "!v.has_custom_item" }, { "brandish_spear" : "v.is_brandishing_spear" }, { "holding_spyglass": "v.is_holding_spyglass" }, { "charging" : "q.is_charging" }, { "sneaking" : "q.is_sneaking && !q.is_sleeping" }, { "bob": "!v.is_holding_spyglass" }, { "damage_nearby_mobs" : "v.damage_nearby_mobs" }, { "swimming" : "v.swim_amount > 0.0" }, { "swimming.legs" : "v.swim_amount > 0.0" }, { "use_item_progress" : "( v.use_item_interval_progress > 0.0 ) || ( v.use_item_startup_progress > 0.0 ) && !v.is_brandishing_spear && !v.is_holding_spyglass && !v.has_custom_item" }, { "sleeping" : "q.is_sleeping && q.is_alive" }, { "attack.positions" : "v.attack_time >= 0.0" }, { "attack.rotations" : "v.attack_time >= 0.0 && !v.has_custom_item" }, { "shield_block_main_hand" : "q.blocking && q.get_equipped_item_name('off_hand') != 'shield' && q.get_equipped_item_name == 'shield'" }, { "shield_block_off_hand" : "q.blocking && q.get_equipped_item_name('off_hand') == 'shield'" }, { "crossbow_controller" : "q.get_equipped_item_name == 'crossbow'" }, { "third_person_bow_equipped" : "q.get_equipped_item_name == 'bow' && (v.item_use_normalized > 0 && v.item_use_normalized < 1.0) " } ], "transitions" : [ { "paperdoll" : "v.is_paperdoll" }, { "first_person" : "v.is_first_person" }, { "map_player" : "v.map_face_icon" } ] }, "first_person_transition" : { "transitions" : [ { "first_person" : "1" } ], "blend_via_shortest_path": true }, "third_person_transition" : { "transitions" : [ { "third_person" : "1" } ], "blend_via_shortest_path": true } } })");
			...
			rp_anim["animations"]["animation.player.custom_item.base_first_person_pose"] = nlohmann::ordered_json::parse(R"({ "loop": true, "bones": { "root": { "rotation": [0, 180, 0], "position": [0, 0, 0] }, "leftArm": { "position": [10, 0, -5] }, "rightArm": { "position": [-10, 0, -5] } } })");
			rp_anim["animations"]["animation.player.custom_item.third_person_aim_arm.right"] = nlohmann::ordered_json::parse(R"({ "loop": true, "bones": { "rightArm": { "rotation": ["q.target_x_rotation - (q.is_sneaking * 15) ", "q.target_y_rotation", 0] } } })");
			rp_anim["animations"]["animation.player.custom_item.third_person_aim_arm.left"] = nlohmann::ordered_json::parse(R"({ "loop": true, "bones": { "leftArm": { "rotation": ["q.target_x_rotation - (q.is_sneaking * 15) ", "q.target_y_rotation", 0] } } })");
...
void entity::add_custom_item_entry(const std::string& item_name, const int& indent)
{
	std::string shortname = utilities::split(item_name, ':').back();

	// Player Entity
	nlohmann::ordered_json rp_entity = file_manager::read_json_from_web_page("https://raw.githubusercontent.com/Mojang/bedrock-samples/main/resource_pack/entity/player.entity.json");
	rp_entity = file_manager::read_json_from_file(file_manager::get_rp_path() + "\\entity\\player.entity.json", rp_entity);
	rp_entity["minecraft:client_entity"]["description"]["animations"]["ctrl." + shortname] = "controller.animation.player.custom_items." + shortname;
	...
	std::string has_custom = "v.has_custom_item = v." + shortname + ";";
	size_t index = 10;
	for (size_t i = 0; i < pre_anim.size(); i++)
	{
		if (pre_anim[i].find("has_custom_item") != std::string::npos)
		{
	...
	nlohmann::ordered_json rp_anim_controller = file_manager::read_json_from_file(file_manager::get_rp_path() + "\\animation_controllers\\player.ac.json", rp_player_ac_default);
	rp_anim_controller["animation_controllers"]["controller.animation.player.custom_item.select"]["states"]["no_item"]["transitions"].push_back({ { shortname, "v." + shortname } });
	std::vector<nlohmann::ordered_json> transitions = rp_anim_controller["animation_controllers"]["controller.animation.player.custom_item.select"]["states"]["no_item"]["transitions"];
	transitions.insert(transitions.begin(), { {{"no_item", "!v.has_custom_item"}} });
	rp_anim_controller["animation_controllers"]["controller.animation.player.custom_item.select"]["states"][shortname]["animations"].push_back("ctrl." + shortname);
	rp_anim_controller["animation_controllers"]["controller.animation.player.custom_item.select"]["states"][shortname]["transitions"] = transitions;

	for (auto& item : rp_anim_controller["animation_controllers"]["controller.animation.player.custom_item.select"]["states"].items())
	{
		if (item.key() != "no_item")
		{
			size_t new_item = 0;
			...
			temp_transitions.erase(temp_transitions.begin() + new_item);
	...
	// Weapon Animation
	rp_anim = file_manager::read_json_from_file(file_manager::get_rp_path() + "\\animations\\item.anim.json", rp_player_anim);
	if (!rp_anim["animations"].contains("animation.item." + shortname + ".idle.first_person")) rp_anim["animations"]["animation.item." + shortname + ".idle.first_person"] = nlohmann::json::object();
	if (!rp_anim["animations"].contains("animation.item." + shortname + ".idle.third_person")) rp_anim["animations"]["animation.item." + shortname + ".idle.third_person"] = nlohmann::json::object();
	if (!rp_anim["animations"].contains("animation.item." + shortname + ".attack.first_person")) rp_anim["animations"]["animation.item." + shortname + ".attack.first_person"] = nlohmann::json::object();
	if (!rp_anim["animations"].contains("animation.item." + shortname + ".attack.third_person")) rp_anim["animations"]["animation.item." + shortname + ".attack.third_person"] = nlohmann::json::object();
	file_manager::write_json_to_file(rp_anim, file_manager::get_rp_path() + "\\animations\\item.anim.json", indent);
}
